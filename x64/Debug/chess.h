...
using namespace sf;
Board game;
bool over, returnToMainMenu;
PieceColor turn = White;
Sprite background;
bool twoPlayers;
Font font;
Sprite modeSelection, colorSelection, endScreen, diffcultyScreen, turnHighlight;
Text wName, bName, scoreB, scoreW;
Sprite promoteBlack, promoteWhite;
Texture checkers;
Texture turnTex;
Texture modeTex, colorTex, endTex, diffcultyTex;
stack<Moves>history;
...
int whiteScore, blackScore, diffculty;
...
//Lets the user choose the computer's diffculty
void chooseDiffculty(RenderWindow &win) {
	win.draw(diffcultyScreen);
	win.display();
	int actions = 0;
	while (actions < 1) {
		Event ev;
		while (win.pollEvent(ev)) {
			if (ev.type == Event::MouseButtonReleased && ev.mouseButton.button == Mouse::Left) {
				if (ev.mouseButton.y >= 335 && ev.mouseButton.y <= 380 && ev.mouseButton.x >= 231 && ev.mouseButton.x <= 290) {
					diffculty = 1;
					actions++;
				}
				else if (ev.mouseButton.y >= 335 && ev.mouseButton.y <= 380 && ev.mouseButton.x >= 324 && ev.mouseButton.x <= 382) {
					diffculty = 2;
					actions++;
				}
				else if (ev.mouseButton.y >= 335 && ev.mouseButton.y <= 380 && ev.mouseButton.x >= 417 && ev.mouseButton.x <= 476) {
					diffculty = 3;
					actions++;
				}
				else if (ev.mouseButton.y >= 335 && ev.mouseButton.y <= 380 && ev.mouseButton.x >= 509 && ev.mouseButton.x <= 567) {
					diffculty = 4;
					actions++;
				}
			}
...
void globalIn(RenderWindow &win) {
	...
	diffcultyTex.loadFromFile("graphics\\chess-window2.png");
	diffcultyScreen.setTexture(diffcultyTex);
	over = false;
	...
	chooseColor(win);
	if (firstPlayerC == White){
		checkers.loadFromFile("graphics\\ChessBoard2.png");
	}
	else {
		checkers.loadFromFile("graphics\\ChessBoard1.png");
	}
...
short isCastling(Moves choice) {
	if (choice.from.first == 7 && choice.from.second == 4 && game.board[choice.from.first][choice.from.second]->pieceType == king) {
	...
	else if (choice.from.first == 0 && choice.from.second == 4 && game.board[choice.from.first][choice.from.second]->pieceType == king) {
...
void undo() {
	if (history.empty() || (history.top().movetype != Capture && history.top().movetype != RegularMove)) return;
	game.reverseMove(history.top());
	history.pop();
	if (!twoPlayers&&history.size()) {
		game.reverseMove(history.top());
		history.pop();
	}
	if (twoPlayers) {
...
//Checks if there are no legal moves to make
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (game.board[i][j]->pieceColor == turn) {
				res += game.board[i][j]->AvailableMoves(game.boardColor, game.boardType).size();
			}
		}
	}
	return !(res>0);
}
// The function that checks the game state
//Whether if one of the players win, or a draw
GameState checkState(RenderWindow &win) {
	game.getBoardColor();
	game.getBoardType();
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
...
void calcScore() {
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			...
			else if (game.board[i][j]->pieceColor == White) {
...
//Checks if a pawn should be promoted, if a pawn is found, it asks the user to choose a piece to promote to
//Promotes said piece and saves the order in the history
...
// The function that displays and handle the input after the game is finished
...
//The main function that handles the flow of the game
//Checking the game state, saving the history, moderating turns
//And handling the winning screens
void chess_main(RenderWindow &win) {
	Clock clock;
	win.create(VideoMode(800, 640), "Chess");
	globalIn(win);
	while (win.isOpen()) {
		GameState curGameState = checkState(win);
		if (curGameState != Running) {
			if ( curGameState==Draw && !over) {
				win.draw(gameDraw);
				win.display();
				over = true;
			}
			...
			if (turn == White && curGameState == Running) {
				...
				if (firstPlayerC == Black &&!twoPlayers) {
					Moves aiMove = AI.makemove(game, turn, diffculty);
					short castleFlag = isCastling(aiMove);
					if (castleFlag) {
						pair<Moves, Moves> temp = Castle(castleFlag);
						game.recordMove(temp.first);
						game.recordMove(temp.second);
